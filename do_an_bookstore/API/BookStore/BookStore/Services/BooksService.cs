        ...
        public async Task<object> GetAllSanPhamSearch(string keyword, int page)
        {
            ...
            var query = await (from bk in _context.Books
                              ...
                              select new
                              {
                                  ...
                                  grouped.Key.Price,
                                  tenTacGia = string.Join(", ", grouped.Select(x => x.FullName).ToArray()),
                              }).ToListAsync();
            var totalRecords = query.Where(x => x.tenTacGia.ToLower().Contains(keyword.ToLower()) || x.Title.ToLower().Contains(keyword.ToLower())).Count();

            var book = query.Where(x => x.tenTacGia.ToLower().Contains(keyword.ToLower()) || x.Title.ToLower().Contains(keyword.ToLower())).Skip(page * 20).Take(20);
            //var book = await (from bk in _context.Books
            //                  join ba in _context.BookAuthors on bk.Id equals ba.BookId into baGroup
            //                  from ba in baGroup.DefaultIfEmpty()
            //                  join at in _context.Authors on ba.AuthorId equals at.Id into atGroup
